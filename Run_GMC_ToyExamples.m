...
%     data{i} = X{i}';
% end
[predY, obj, U, S0, S0_initial] = ToyExample(X, c, 1);
...
for v = 1:m
    ...
    if flag
        plot(X{v}(lab==cLab(3),1),X{v}(lab==cLab(3),2),'.', 'Color', [79 1 79]/255, 'MarkerSize', markerSize); hold on;
    end;
...
for v = 1:m
    ...
    figure; 
    plot(X{v}(:,1),X{v}(:,2),'.k', 'MarkerSize', markerSize); hold on;
    plot(X{v}(lab==cLab(1),1),X{v}(lab==cLab(1),2),'.b', 'MarkerSize', markerSize); hold on;
    plot(X{v}(lab==cLab(2),1),X{v}(lab==cLab(2),2),'.y', 'MarkerSize', markerSize); hold on;
    if flag
    ...
    for ii = 1 : num
        for jj = 1 : ii
            weight = S1{v}(ii, jj);
            if weight > 0
                plot([X{v}(ii, 1), X{v}(jj, 1)], [X{v}(ii, 2), X{v}(jj, 2)], '-', 'Color', [11 197 205]/255, 'LineWidth', 5*weight), hold on;
            end;
        end;
    end;
%     set(gca,'xlim',[-1.7,1.7],'xtick',[-1.5:0.5:1.5]) % set x-axis
